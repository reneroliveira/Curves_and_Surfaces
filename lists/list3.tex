...
\end{bmatrix}\right)$	
	...
	\item[\ex{5}] \textcolor{blue}{Uma aplicação $\Phi:\rr\to\rr$ é dita \textit{movimento rígido} quando preserva distâncias. Isto é:}
	\blue{
	$$||\Phi(p)-\Phi(q)||=||p-q||$$
	...
	\item[\sol{5}] $\Phi:\rr\to\rr$ é diferenciável se, dados $p\in\rr,~v\in\rr$, existe uma transformação linear representada pela matriz $T$, tal que 
	
	$$\Phi(p+v)-\Phi(v)=Tv+r(v),\text{ onde }\lim_{v\to0}\dfrac{r(v)}{||v||}=0$$
	
	$T$ é chamada de derivada de $\Phi$.
	
	Dado $\Phi(p)=Ap+p_0$, tome $T=A$, vejamos que de fato a proriedade de diferenciabilidade é satisfeita:
	
	\begin{align*}
		\Phi(p+v)-\Phi(v)&=A(p+v)-Av\\
		&=Ap+Av-Av\\
		&=Ap´
	\end{align*}

	Como $Ap=Tp$ é direto o fato que $r(v)\equiv0$ e portanto $\lim_{v\to0}\dfrac{r(v)}{||v||}=0$
	
	Logo $\Phi'(p)=A$.
	
	Como a derivada é constante em relação à $p$, segue que $\Phi''(p)=0$.
	
	\item[\ex{6}] \textcolor{blue}{Mostre que uma matriz de rotação e uma matriz de reflexão são aplicações lineares e ortogonais e, portanto, podem ser interpretadas como um movimento rígido.}

	\item[\sol{6}] Uma matriz $A\in\rn$ é ortogonal se $AA^T=I_n$ onde $I_n$ é a matriz identidade de $n$ dimensões. Considerando o $\rr$, uma matriz de rotação é da forma:
