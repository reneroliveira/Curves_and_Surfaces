...
%	&\text{Pelo Teorema da Função Inversa, $h'(t)=1/||\alpha'(t)||$}
	...
	\begin{enumerate}[(a)]
		...
		Utilizando as mesmas notações e definições da questão 6, foi concluído que:
		
		\begin{align}
			\alpha''(t)=\dfrac{d^2s}{dt^2}T_\beta+\dfrac{ds}{dt}T'_\beta\label{alp}
		\end{align}	
		
		O plano osculador de $\alpha$ em $t$ é o plano gerado pelos vetores $T_\beta(s)$ e $N_\beta(s)=\dfrac{T'_\beta}{||T'_\beta||}$.
		
		Além disso, temos que $\dfrac{ds}{dt}=||\alpha'(t)||$ e $\dfrac{d^2s}{dt}=\dfrac{d}{dt}||\alpha'(t)||$. Podemos entender a norma da velocidade com uma função $I\to\real^+$, assim sua derivada em relação a $t$ será uma função escalar.
		
		Dessa forma, pela expressão \ref{alp}, $\alpha''$ é uma combinação linar dos vetores $T_\beta$ e $N_\beta$, implicando que pertence ao plano osculador, que é gerado por esses dois vetores.
		\blue{
